...
- **UI = viewport** (was Electron): gpui app rendering engine state. Talks the same typed RPC whether the engine is in-process or a separate daemon. Organized around **spaces** — (device, folder) pairs, local or synced according to the active profile. The sidebar is the data: an attention-sorted Sessions list (sessions needing attention float to the top), filtered by a searchable spaces dropdown ("All spaces" included) that also hosts space management. The main area is the **workspace**: split panes and tab groups (a persisted layout tree). A sidebar click opens/focuses a session in the active pane; dragging a sidebar row onto a pane edge splits it beside that pane, dropping on the pane center opens it in place, and Cmd/Ctrl-click (or the row menu's "Open in split") does the same without the drag — a session already open in a pane is focused rather than duplicated. Archiving stays an explicit sidebar action. The new-session canvas carries a space picker (defaulting to the sidebar filter, else the last selected space); new sessions are minted onto the picked space's device via relay-forwardable RPCs.
