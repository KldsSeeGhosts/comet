...
`package-macos.sh` and `package-linux.sh` bundle Chromium automatically. On Linux the installer puts the executable and browser directory under `~/.local/lib/noches`, with a launcher symlink in `~/.local/bin`. The runtime must stay next to the real executable. CEF and Chromium license notices ship with it.
