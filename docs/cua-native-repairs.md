...
The shipping Linux seat repair is `vendor/gpui_linux` on the current `zeronsh/zui` pin (`c2d273d`). Do not regenerate it with `scripts/cua/native/vendor_zui.py`; that helper still describes the older `07fd941` vendor and refuses to overwrite a tree that already has one.

`scripts/cua/build_native.sh` patches and builds the separate Cua driver. It does not rebuild Noches and does not read a local gpui overlay. `--install` links `~/.local/bin/cua-driver`.

The notes below are the original repair record.
