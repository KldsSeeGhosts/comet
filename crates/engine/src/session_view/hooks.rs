...
use zeron_shell_hooks::{
    Binding, GenerationOptions, HookEnvelope, HookReceiver, Outcome, Peer, Provider, TurnEvent,
};
...
impl HookRuntime {
    ...
    pub fn prepare(
        ...
        let binding = Binding {
            ...
            provider,
            provider_session_id: Some(native_id.into()),
        };
        ...
        let result = zeron_shell_hooks::generate(
            &registration,
            &GenerationOptions {
                parent_dir: self.root.path().canonicalize()?,
                hook_socket: self.socket.clone(),
                python: python.to_path_buf(),
                provider_executable: PathBuf::from(&command.program),
                instructions: String::new(),
                history_path: Some(command.history_path.clone()),
                path_prefix: vec![],
                notifier_argv: None,
            },
        );
        match result {
            Ok(generated) => {
                state.sessions.insert(
                    chat.into(),
                    Registered {
                        activity: NativeActivity::Unknown,
                        frozen: false,
    ...
    pub fn freeze_idle(&self, chat: &str) -> Result<(), EngineError> {
        ...
        match session.activity {
            NativeActivity::Idle if !session.frozen => {
                session.frozen = true;
                Ok(())
            }
...
fn accept(state: &mut State, token: &str, body: &[u8]) -> Result<(), EngineError> {
    ...
    session.activity = match event.kind {
        ...
        TurnEvent::TurnCompleted
            if envelope.payload["is_idle"] == true
                && !event.demoted
                && !event.background_unknown
                && event.background_tasks == 0 =>
        {
            NativeActivity::Idle
        }
        ...
        | TurnEvent::RunStarted
        | TurnEvent::ToolStarted
        | TurnEvent::SubagentStarted
        | TurnEvent::PermissionResolved => NativeActivity::Busy,
        _ => session.activity,
    };
    Ok(())
}
...
async fn serve(mut stream: UnixStream, state: Arc<Mutex<State>>) -> Result<(), EngineError> {
    if stream.peer_cred()?.uid() != unsafe { libc::geteuid() } {
        return Err(failed("Hook peer UID mismatch"));
    }
    ...
    let end = loop {
        ...
        if n == 0 {
            return Err(failed("Incomplete hook request"));
        }
        bytes.extend_from_slice(&buffer[..n]);
        if bytes.len() > 73728 {
            return Err(failed("Hook request too large"));
        }
        if let Some(end) = bytes.windows(4).position(|w| w == b"\r\n\r\n") {
            break end;
        }
        if bytes.len() > 8192 {
            return Err(failed("Hook headers too large"));
        }
    };
    ...
    for line in header.lines().skip(1) {
        let (key, value) = line
            .split_once(':')
            .ok_or_else(|| failed("Invalid hook header"))?;
        match key.to_ascii_lowercase().as_str() {
            "content-length" => {
                if length.is_some() {
                    return Err(failed("Duplicate content length"));
                }
                length = Some(
                    value
                        .trim()
            ...
            "authorization" => {
                if token.is_some() {
                    return Err(failed("Duplicate authorization"));
                }
...
mod tests {
    ...
    fn only_providers_with_verified_admission_and_settlement_are_managed() {
        ...
        for harness in [
            ...
            assert!(
                managed_provider(harness).is_err(),
                "{harness:?} has no verified lifecycle adapter"
            );
        }
    }
    ...
    async fn lifecycle_authentication_and_freeze_are_atomic() {
        ...
        let binding = Binding {
            ...
            worktree_path: runtime.root.path().canonicalize().unwrap(),
        ...
        state.sessions.insert(
            ...
            Registered {
                activity: NativeActivity::Unknown,
                frozen: false,
                directory: runtime.root.path().join("unused"),
            },
        );
        let body = |name, id| {
            serde_json::to_vec(&HookEnvelope {
                ...
                event_id: id,
                event_name: name,
                payload: json!({"session_id":"native", "is_idle":true}),
            })
            .unwrap()
        };
        assert!(
            accept(
                &mut state,
                &"0".repeat(64),
                &body("session_start".into(), "0".into())
            )
            .is_err()
        );
        ...
        accept(
            &mut state,
            &registration.token,
            &body("session_start".into(), "1".into()),
        )
        .unwrap();
        accept(
            &mut state,
            &registration.token,
            &body("before_agent_start".into(), "2".into()),
        )
        .unwrap();
        drop(state);
        assert!(runtime.freeze_idle("chat").is_err());
        accept(
            &mut lock(&runtime.state),
            &registration.token,
            &body("ui_prompt_start".into(), "permission".into()),
        )
        .unwrap();
        ...
        let mut pending: serde_json::Value =
            serde_json::from_slice(&body("agent_settled".into(), "pending".into())).unwrap();
        ...
        accept(
            &mut lock(&runtime.state),
            &registration.token,
            &body("agent_settled".into(), "3".into()),
        )
        .unwrap();
        ...
        assert!(
            accept(
                &mut lock(&runtime.state),
                &registration.token,
                &body("before_agent_start".into(), "4".into())
            )
            .is_err()
        );
        runtime.revoke("chat");
