...
pub fn app_menus() -> Vec<Menu> {
    ...
    let mut menus = vec![
        Menu::new("Noches").items(app_items),
...
mod tests {
    ...
    fn app_menu_ends_with_quit() {
        let menus = app_menus();
        assert_eq!(menus[0].name.as_ref(), "Noches");
...
// (`cargo run`) show "Noches" and the crate version instead of the process name.
#[cfg(target_os = "macos")]
mod about_panel {
    ...
    pub(super) fn show() {
        ...
        let name = NSString::from_str("Noches");
