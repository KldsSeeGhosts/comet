    ...
    pub fn setup_description(self) -> &'static str {
        match self.platform {
            ...
            AppshotPlatform::LinuxWayland => {
                "Your desktop owns capture and shortcut consent. Noches checks each portal capability separately and explains any required fallback."
            }
            AppshotPlatform::LinuxX11 => {
                "X11 normally needs no capture permission. Noches prefers an active-window screenshot portal when available and otherwise uses native X11 capture."
            }
    ...
    pub fn capture_description(self) -> &'static str {
        match (self.platform, self.target) {
            (AppshotPlatform::MacOs, _) => {
                "Screen Recording lets Noches capture the frontmost window. macOS may request one restart."
            }
            (AppshotPlatform::LinuxX11, _) => {
                "Noches uses native X11 capture when active-window portal capture is unavailable. Obscured or protected windows may be incomplete."
            }
...
pub fn validate_capture_dimensions(width: u32, height: u32) -> Result<usize, CaptureError> {
    ...
    if pixels > MAX_CAPTURE_PIXELS || rgba_bytes > MAX_CAPTURE_RGBA_BYTES {
        return Err(CaptureError::CaptureFailed(format!(
            "The captured window ({width}×{height}) exceeds Noches' capture budget."
        )));
    }
...
fn trim_appshot_padding(bytes: &[u8]) -> Result<Option<Vec<u8>>, CaptureError> {
    ...
    if bytes.len() as u64 > crate::attachments::MAX_ATTACHMENT_BYTES {
        return Err(CaptureError::CaptureFailed(
            "The captured window is larger than Noches' 24 MB image limit.".into(),
        ));
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::PermissionRequired => f.write_str(
                "Window capture permission is required. Open Noches Settings > Appshots for the platform-specific recovery step.",
            ),
