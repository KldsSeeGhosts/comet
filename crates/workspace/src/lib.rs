...
pub struct PaneState {
    ...
    /// Super tab object's `provider_key` (e.g. "codex", "pi").
    #[serde(default)]
    pub provider_key: Option<String>,
    /// Super tab object's `conversation_id`.
    #[serde(default)]
    pub conversation_id: Option<String>,
    /// Super tab object's `permission_mode` (e.g. "bypass").
    #[serde(default)]
    pub permission_mode: Option<String>,
    /// Whether `label` was set by Super's `sc`-owned title (`title_sc_owned`).
    #[serde(default)]
    pub title_sc_owned: bool,
    /// Catch-all for Super tab-object fields with no engine counterpart
    /// (`tab_uuid`, `messages_snapshot`, `thinking_enabled`,
    /// `provider_profile_selection`, terminal `preset_key` /
    /// `working_directory_path`, ...). Keeps the `super_format` bridge and
    /// engine-native persistence lossless. `deny_unknown_fields` cannot be
    /// combined with `flatten`, so unknown pane fields are absorbed here
    /// instead of rejected.
...
pub struct WorkspaceLayout {
    ...
    pub next_id: u64,
    pub revision: u64,
    /// Catch-all for foreign worktree-level keys (Super's `file_tree_*`,
    /// `change_list_*`, `diff_*`, ...), letting the `super_format` bridge
    /// round-trip a persisted Super value losslessly. `flatten` precludes
    /// `deny_unknown_fields`, so unknown top-level fields are absorbed here
    /// instead of rejected.
    #[serde(default, flatten)]
    pub extra: serde_json::Map<String, serde_json::Value>,
}
...
impl WorkspaceLayout {
    ...
    pub fn move_pane(&mut self, pane: PaneId, target: PaneId, direction: Direction) -> Result<()> {
        self.compose(self.revision, |draft| {
            ...
            let should_swap = draft.pane_location(pane).is_some_and(|(sv, st)| {
                sv == view
                    && st == tab
                    && draft.views[&view].tabs[&tab]
                        .root
                        .is_sibling_in_direction(pane, target, direction)
            });
            if should_swap {
                tree::swap_leaves(
                    &mut draft
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for (view_id, view) in &self.views {
            ...
            if order.len() != view.tabs.len()
                || order.iter().copied().collect::<BTreeSet<_>>()
                    != view.tabs.keys().copied().collect()
            {
                return Err(LayoutError::Invalid(
                    "tab order must contain each tab exactly once",
                ));
            }
            tabs += view.tabs.len();
            if tabs > MAX_TABS {
