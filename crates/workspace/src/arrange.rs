use crate::{
    Direction, LayoutError, PaneId, Result, SplitNode, SplitTabLayout, TabId, ViewId,
    WorkspaceLayout,
};
...
fn replace_with_split(
    node: &mut SplitNode<PaneId>,
    target: PaneId,
    added: &SplitNode<PaneId>,
    direction: Direction,
) -> bool {
    match node {
        ...
        SplitNode::Split { first, second, .. } => {
            replace_with_split(first, target, added, direction)
                || replace_with_split(second, target, added, direction)
        }
    }
}
...
impl WorkspaceLayout {
    ...
    pub fn tab_location(&self, tab: TabId) -> Option<ViewId> {
        self.views
            .iter()
            .find_map(|(id, view)| view.tabs.contains_key(&tab).then_some(*id))
    }
    ...
    pub fn swap_panes(&mut self, a: PaneId, b: PaneId) -> Result<()> {
        if a == b {
            return Err(LayoutError::Invalid("swap source equals target"));
        }
        self.compose(self.revision, |draft| {
            let (av, at) = draft
                .pane_location(a)
                .ok_or(LayoutError::NotFound("source pane"))?;
            let (bv, bt) = draft
                .pane_location(b)
                .ok_or(LayoutError::NotFound("target pane"))?;
            let active = draft
                .active_pane_id()
                .ok_or(LayoutError::NotFound("active pane"))?;
            if (av, at) != (bv, bt) {
    ...
    pub fn move_tab(&mut self, tab: TabId, target: ViewId) -> Result<()> {
        self.compose(self.revision, |draft| {
            ...
            if !draft.views.contains_key(&target) {
                return Err(LayoutError::NotFound("target view"));
            }
            if source != target {
    ...
    pub fn pane_to_tab(&mut self, pane: PaneId, target: ViewId) -> Result<TabId> {
        self.compose(self.revision, |draft| {
            ...
            draft
                ...
                .get_mut(&target)
                .unwrap()
                .insert_tab(tab, SplitTabLayout::single(pane, state));
    ...
    pub fn reorder_tab(&mut self, tab: TabId, target: ViewId, before: Option<TabId>) -> Result<()> {
        self.compose(self.revision, |draft| {
            if before == Some(tab) && draft.tab_location(tab) == Some(target) {
                return Ok(());
            }
    ...
    pub fn tab_to_view(
        ...
        self.compose(self.revision, |draft| {
            if draft.tab_location(tab).is_none() {
    ...
    pub fn merge_tab(&mut self, tab: TabId, target: PaneId, direction: Direction) -> Result<()> {
        self.compose(self.revision, |draft| {
            let source = draft
                .tab_location(tab)
                .ok_or(LayoutError::NotFound("tab"))?;
            let (view, destination) = draft
                .pane_location(target)
                .ok_or(LayoutError::NotFound("target pane"))?;
            if destination == tab {
                return Err(LayoutError::Invalid("cannot merge a tab into itself"));
            }
...
mod tests {
    ...
    fn whole_tab_moves_and_merges_without_losing_children() {
        ...
        let source_view = layout
            .split_view(destination_view, Direction::Right, PaneState::default())
            .unwrap();
        let first = layout.active_pane_id().unwrap();
        let second = layout
            .split_pane(first, Direction::Down, PaneState::default())
            .unwrap();
        ...
        let destination =
            &layout.views[&destination_view].tabs[&layout.views[&destination_view].active_tab_id];
        assert_eq!(destination.panes.len(), 3);
        let SplitNode::Split { first, .. } = &destination.root else {
    ...
    fn pane_can_become_tab_then_full_view_without_identity_changes() {
        ...
        let first = layout.active_pane_id().unwrap();
        let second = layout
            .split_pane(first, Direction::Right, PaneState::default())
            .unwrap();
